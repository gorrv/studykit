  ...
  function runCurry() {
    ...
    html += '</div>';
    if (err) {
      html += `<div class="verdict bad">✗ ${err}<br><br>This is what is meant by "<em>multiple arguments can be passed as one in a tuple, but this does not allow partial application</em>": a tuple argument is <strong>one</strong> argument, so there is no intermediate stage to stop at.</div>`;
    } else if (cur.k === 'fun') {
