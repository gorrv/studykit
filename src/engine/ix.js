  ...
  function ixTrace(id, outId, opts) {
    ...
    for (var q = 0; q < rows.length; q++) {
      total = Math.max(total, Number(rows[q].getAttribute('data-step')) + 1);
    }
    ...
    /* The transport bar's buttons are inline onclick handlers, so each player
       needs a named global. Define it here rather than making every tool
       hand-write the same one-line wrapper. */
    if (typeof window[id + 'Step'] !== 'function') {
      window[id + 'Step'] = function (d, s) { ixStep(id, d, s); };
    }
  ...
  function ixStep(id, d, fromSlider) {
    ...
    if (d === 'play') {
      IX.play(id, h.total,
        function () { return IX.frames[id]; },
        function (f) { IX.frames[id] = f; ixPaint(id); }, 650);
      ixPaint(id);
      return;
    }
    IX.stop(id);
