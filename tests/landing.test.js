...
const { Suite, ROOT } = require('./lib/harness');
...
module.exports = async function run() {
  ...
  cards.forEach(a => {
    ...
    s.ok(`card links to a real file: ${href}`, fs.existsSync(path.join(ROOT, href)));
  });

  // Every module on disk should be listed, so none silently goes missing.
  const onDisk = fs.readdirSync(path.join(ROOT, 'modules')).filter(f => f.endsWith('.html'));
